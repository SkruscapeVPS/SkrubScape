...
public class Player extends Entity {
	...
	public boolean inZeah() {
		if (absX > 1450 && absX < 1900 && absY > 3400 && absY < 3930) {
			return true;
		} else
			return false;
	}
	...
	public void GetDice() {
		...
		for (int z = 0; z < PlayerHandler.players.length; z++) {
			if (PlayerHandler.players[z] != null) {
				Player o = (Player) PlayerHandler.players[z];
	...
	public boolean checkPacket126Update(String text, int id) {
		...
		if (interfaceText.containsKey(id)) {
			...
			if (t != null && text.equals(t.currentState)) {
				return false;
			}
		}
    ...
    public void highscores() {
        getPA().sendFrame126("Noxious PK Board", 42002);  //Title
        for(int i = 0; i < 10; i++) {
                if(ranks[i] > 0) {
	...
	public void loadHighscore() {
		getPA().sendFrame126("Noxious - Top PKers Online", 6399);
		...
		getPA().sendFrame126("Noxious ", 640);
	...
	public void initialize() {
		try {
			...
			sendMessage("Welcome to <col=255>SkrubScape!");
								...
								new int[] { 250000, 1, 1, 1, 50, 10, 5, 5, 1 }, new StartSet() {
									public void giveStarter(Player client) {
										...
										client.sendMessage("[<img=10><col=255>Info</col>] "
												+ "</col>Welcome to SkrubScape, we are a Eco/Pk server");
										client.sendMessage("[<img=10><col=255>Info</col>] "
	...
	public abstract static class StartSet {
		...
		public boolean checkReq(Player player) {
			...
			player.sendMessage("[<img=10><col=255>Info</col>] " + "</col>Welcome to SkrubScape, an OSRS based server!");
	...
	public void process() {
		...
		getPA().sendFrame126("Players Online: @or1@" + PlayerHandler.getPlayersOnline() + "", 29163);
		...
		if (secondsPlayed >= 100) {
			minutesPlayed += 1;
			getPA().sendFrame126("@or1@Days: @whi@" + daysPlayed + "@or1@ Hrs: @whi@" + hoursPlayed
					+ "@or1@ Mins: @whi@" + minutesPlayed + "", 29162);
			secondsPlayed = 0;
		}
		if (minutesPlayed == 60) {
			hoursPlayed += 1;
		...
		} else if (inClanWars()  || inGEPK() || (hungerGames && !isNpc && HungerManager.getSingleton().gameRunning()) || inFunPk()) {
			getPA().showOption(3, 0, "Attack", 1);
		} else if (inEdgeville()) {
			if (Config.BOUNTY_HUNTER_ACTIVE) {
	...
	public boolean isInMole() {
		if (absX > 2998 && absX < 3001 && absY > 3382 && absY < 3384) {
			return true;
		}
		return false;
	}
	...
	public boolean inWild() { // MY VERSION
		if (absX > 2941 && absX < 3392 && absY > 3524 && absY < 3966
				...
				|| absX > 3159 && absX < 3173 && absY > 3450 && absY < 3465) {
...
public boolean inGEPK () {
	return Boundary.isIn(this, Config.GE_PK_BOUNDARY);
}
	...
	public boolean inDice() {
		if (absX > 2212 && absX < 2233 && absY > 3782 && absY < 3815) {
			// if (absX > 0 && absX < 0 && absY > 0 && absY < 0) {
			return true;
		}
		return false;
	}
	...
	public boolean checkFullGear(Player c) {
		if (c.playerEquipment[0] >= 0 && c.playerEquipment[1] >= 0 && c.playerEquipment[2] >= 0
	...
	public boolean hasBoneCrusher() {
		if (getItems().playerHasItem(13116)) {
			return true;
		}
		return false;
	}

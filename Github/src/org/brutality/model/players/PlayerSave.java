...
public class PlayerSave {
			...
			if (spot > -1) {
				...
				case 2:
					if (token.equals("character-height")) {
						...
						p.attack = Boolean.parseBoolean(token2);
					} else if (token.equals("strength")) {
						p.strength = Boolean.parseBoolean(token2);
					} else if (token.equals("hitpoint")) {
						p.hitpoints = Boolean.parseBoolean(token2);
					} else if (token.equals("range")) {
						p.attack = Boolean.parseBoolean(token2);
					} else if (token.equals("magic")) {
						p.attack = Boolean.parseBoolean(token2);
					} else if (token.equals("prayer")) {
						p.prayerLevel = Boolean.parseBoolean(token2);
					} else if (token.equals("defence")) {
						p.defence = Boolean.parseBoolean(token2);
	...
	public static boolean saveGame(Player p) {
		...
		try {
			...
			characterfile.write(Boolean.toString(p.attack), 0, Boolean.toString(p.attack).length());
			characterfile.newLine();
			characterfile.write("strength = ", 0, 11);
			characterfile.write(Boolean.toString(p.strength), 0, Boolean.toString(p.strength).length());
			characterfile.newLine();
			characterfile.write("defence = ", 0, 10);
			characterfile.write(Boolean.toString(p.defence), 0, Boolean.toString(p.defence).length());
			characterfile.newLine();
			characterfile.write("range = ", 0, 8);
			characterfile.write(Boolean.toString(p.range), 0, Boolean.toString(p.range).length());
			characterfile.newLine();
			characterfile.write("prayer = ", 0, 9);
			characterfile.write(Boolean.toString(p.prayerLevel), 0, Boolean.toString(p.prayerLevel).length());
			characterfile.newLine();
			characterfile.write("magic = ", 0, 8);
			characterfile.write(Boolean.toString(p.magic), 0, Boolean.toString(p.magic).length());
			characterfile.newLine();
			characterfile.write("hasBankpin = ", 0, 13);
			characterfile.write(Boolean.toString(p.hasBankpin), 0, Boolean.toString(p.hasBankpin).length());
			characterfile.newLine();

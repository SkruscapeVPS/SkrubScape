...
public class RangeData {
		...
		if(c.oldNpcIndex > 0) {
			if(NPCHandler.npcs[c.oldNpcIndex] != null) {
				...
				if (c.lastWeaponUsed == 12926) {
					...
					c.getPA().createPlayersProjectile2(pX, pY, offX, offY, 50, c.getCombat().getProjectileSpeed(), 250, 43, 31, - c.oldNpcIndex + 1, c.getCombat().getStartDelay(), 10);
					c.msbSpec = false;
				}
				int slope;
	...
	public static int getRangeStr(int i) {
		...
		int[][] data = {
			...
			{10034, 15}, {12926, 48}, {19484, 175}, {15041, 250}
	...
	public static int getRangeStartGFX(Player c) {
		...
		int[][] data = {
			...
			{4221, 250}, {4222, 250}, {4223, 250}, {15041, 250},
		};
		for(int l = 0; l < data.length; l++) {
	...
	public static int getRangeProjectileGFX(Player c) {
		...
		if (c.playerEquipment[c.playerWeapon] == 11785 && c.acbSpec)
			return 301;
		...
		int[][] data = {
			...
			{4221, 249}, {4222, 249}, {4223, 249}, {15041, 249},
		};

	...
	public void processPacket(final Player c, Packet packet) {
		...
		case FIRST_CLICK:
			...
			if (c.objectId == 13291) {
				...
				if (c.getItems().playerHasItem(604, 3)) {
					c.sendMessage("You Tele To The Waiting Room! And @red@Exchange Your Shards For A Key!");
					c.getPA().movePlayer(2256, 3177, 0);					
					c.getItems().deleteItem(604, 1);
					c.getItems().deleteItem(604, 1);
					c.getItems().deleteItem(604, 1);
					c.getItems().addItem(2887, 1);
				}
			else c.sendMessage("@red@You Do Not Have The Required Shards!");
				return;
			}
			if (c.objectId == 24603) {
			Server.npcHandler.spawnNpc(c, 6345, 3191, 3962, 0, 1, 155, 20, 350, 250, false, false);
			c.sendMessage("You Have Spawned A New Boss");
			return;
			}
			if (c.objectId == 4150) {
				c.stopMovement();
				c.getPA().walkTo(3190, 3959);
			c.getPA().movePlayer(3093, 3494, 0);
			c.sendMessage("You Have Teleported Away From The Boss Room");
			return;
			}
		if (c.objectId == 4151) {
			if (c.getItems().playerHasItem(12821) || c.getItems().playerHasItem(12817) || c.getItems().playerHasItem(12825)) {
				c.stopMovement();
				c.getPA().walkTo(3089, 3488);
				c.getItems().addItem(15001, 1);
				c.getItems().deleteItem(12821, 1);
				c.getItems().deleteItem(12817, 1);
				c.getItems().deleteItem(12825, 1);
				c.animation(899);
			} else {
				c.sendMessage("@red@You Need An Elysian, Spectral and Arcane Spirit Shield To Create A Divine!");
			return;
		}
		}	
	
			if (c.objectId == 23921) {
				if (c.getItems().playerHasItem(2887, 1)) {
					c.sendMessage("You Climb Up The Shaking Ladder And Seem To Have Lost Your Key..");
					c.getPA().movePlayer(3190, 3959, 0);
					c.getItems().deleteItem(2887, 1);
					c.animation(828);
				}
					else c.sendMessage("You Do Not Have The Required @red@Key!");
				return;
			}
			if(c.objectId == 4308) {
				c.objectDistance = 5;
			...
			case 16530:
				...
				break;
			case 9398:// deposit
				c.getPA().sendFrame126("The Bank of SkrubScape - Deposit Box", 7421);
		...
		case SECOND_CLICK:
			c.objectId = packet.getUnsignedShortA(); //getUnsignedShortA
		...
		case THIRD_CLICK:
			...
			c.objectId = packet.getUnsignedShortA();
			c.objectDistance = 1;
